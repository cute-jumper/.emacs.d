...
(setq god-mod-alist '((nil . "C-")
                      ("g" . "C-M-")
                      ("G" . "M-")))
;; make it usable in minibuffer and silence mode changing messages
(defun qjp-silent-god-local-mode ()
  (interactive)
  (cl-letf (((symbol-function #'message) #'ignore))
    (call-interactively #'god-local-mode)))

(define-key minibuffer-local-map (kbd "<escape>") #'qjp-silent-god-local-mode)
...
;;   (define-key god-local-mode-map (kbd "SPC") #'helm-mini)
;;   (define-key god-local-mode-map (kbd "TAB") #'mode-line-other-buffer)
...
(with-eval-after-load 'helm
  ;; use <escape> to enter `god-mode'
  (define-key helm-map (kbd "<escape>") #'qjp-silent-god-local-mode))
